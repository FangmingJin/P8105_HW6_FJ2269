...
is closed to normal distribution. Its mean is 2.0126, its sd is 0.0236

95% CI for r squared and
log(beta0\*beta1)

``` r
lower_limit_r = quantile(pull(bootstrap_weather,r.squared), probs = c(0.05, 0.95)) %>% 
  .[[1]] %>%
  round(., 3)
upper_limit_r = quantile(pull(bootstrap_weather,r.squared), probs = c(0.05, 0.95)) %>% 
  .[[2]] %>%
  round(., 3)
lower_limit_log = quantile(pull(bootstrap_weather,log), probs = c(0.05, 0.95)) %>% 
  .[[1]] %>%
  round(., 3)
upper_limit_log = quantile(pull(bootstrap_weather,log), probs = c(0.05, 0.95)) %>% 
  .[[2]] %>%
  round(., 3)
```

The 95% CI for r squared is (0.897, 0.925), The 95% CI for
log(beta0\*beta1) is (1.973, 2.05)
